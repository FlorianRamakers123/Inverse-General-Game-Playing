...
def write_csv(csv_data,game):
    f = open(module_path + "/results/incremental/{}_incremental_data.csv".format(game), 'w')
...
def run(games,timeout,timeout_ext,data_set):
    for game in games:
        ...
        for subtarget in predicates:
            train_target_dataset(game,timeout,subtarget, module_path + "/../data/train/{0}/{1}/claudien/{0}_{2}_cwa.dat".format(game,data_set,subtarget))
        ...
        for i,pred in enumerate(predicates):
            ...
            _f = open(module_path + "/../data/train/{0}/{1}/claudien_incremental/{0}_{2}_cwa.dat".format(game,data_set,pred), 'r')
            _content = _f.read()
            _f.close()
        ...
        for repeat in range(REPEAT):
            ...
            for subtarget in predicates:
                ...
                if os.path.exists(pos_ex_path):
                    ...
                    new_pos_examples = [s.strip() for s in pos_f.read().split("% =========")]
                ...
                if os.path.exists(neg_ex_path):
                    ...
                    new_neg_examples = [s.strip() for s in neg_f.read().split("% =========!")]
                ...
                f.write("\n".join(["% =========\n" + s for s in pos_examples]))
                f.write("\n".join(["% =========!\n" + s for s in neg_examples]))
                f.close()
            ...
            for i,pred in enumerate(predicates):
                ...
                csv_data.append([pred,p_and_r[pred][0],p_and_r[pred][1],avg_err_rate,pos_ex_count[pred],neg_ex_count[pred],timeout])
            ...
            if old_nb_neg_examples == new_nb_neg_examples and old_nb_pos_examples == old_nb_neg_examples:
                if avg_err_rate == 0:
                    ...
                    if not prev_extended:
                        print(colored("Extending time out because of empty extension.", "red"))
                        timeout += timeout_ext
                        prev_extended = True
                    else:
                        print(colored("Exiting because maximal extension was reached.", "red"))
                        print("writing csv file")
                        write_csv(csv_data,game)
                        exit()
            else:
